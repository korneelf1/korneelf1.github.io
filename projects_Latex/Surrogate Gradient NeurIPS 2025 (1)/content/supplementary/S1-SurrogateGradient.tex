...
\]
Here, \( \sigma(z) = \frac{1}{1 + e^{-z}} \) is the sigmoid activation. The output layer performs linear population decoding.

\paragraph{Backpropagation and Surrogates.}
During backpropagation, the delta for a neuron \( i \) in the layer \( l \) is:
\[
...
\]
In SNN training, we replace the true derivative \( \sigma'(z) \) with a surrogate gradient:
\[
\Tilde{\sigma}'_k(z) = \frac{\sigma'(kz)}{k^2} = \frac{1}{k} \cdot \sigma(kz) \cdot (1 - \sigma(kz))
\]
where \( k \) controls the slope of the surrogate.  Since the surrogate gradient involves both scaling the input by $k$ (to sharpen the activation) and differentiating with respect to that input, the resulting gradient scales with $k^2$ due to the chain rule. Without compensating for this, gradient magnitudes can explode as $k$ increases. We therefore divide by $k^2$ to stabilize gradient flow regardless of the steepness.”
...
To quantify this, we compute the cosine similarity between gradients using surrogate and true derivatives:
\[
